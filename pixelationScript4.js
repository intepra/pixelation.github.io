...
function drawGraph(){
	...
	console.log("w and h from inputs before: "+w+", "+h);
	...
	if(document.getElementsByName("binHex")[1].checked){ //if the hex radio button is currently selected
		...
		if(DEBUG) console.debug("hex is checked, binCODE:"+binCode);	
	}
	else{
	...
	//	document.getElementById("binaryImage").setSelectionRange(caretPos, caretPos)
		...
		if(DEBUG) console.debug("bin is checked, binCODE:"+binCode);	
	}
	var charIndex=0;
	var errMsg="длинна ";
	...
	// setSliders(); -- want to do this but can't because of recursion.  do by hand instead.
	
	document.getElementById("widthRange").value = w;
	document.getElementById("heightRange").value = h;
	console.log("w and h from inputs after: "+w+", "+h);

	
	bitsPerPix = binToInt(readByte(binCode, 2));
	if(DEBUG) console.debug("read bits per pix: "+bitsPerPix);
	
	document.getElementById("bitsPerPixel").value = bitsPerPix;
	document.getElementById("bitsPerPixelSlider").value = bitsPerPix;

	//if(DEBUG) console.debug("square size="+sqSize)
	...
	//if(DEBUG) console.debug("imgBitString: "+imgBitString);

	var colorNums = bitsToColors(imgBitString,bitsPerPix);
	//if(DEBUG) console.debug("colorNums: "+colorNums);
	
	if(colorNums.length != w*h) errMsg+= "ОШИБКА. ";
	else errMsg+="ok. "
	errMsg += "двоичный:"+colorNums.length+", ШxВ:"+(w*h);
	//document.getElementById("example").innerHTML ="";
	//displayColorList(colorNums);

	var sqSizeMax = parseInt((500/Math.max(w,h))-1);
	sqSizeMax = Math.max(sqSizeMax, 1);
	if(DEBUG) console.debug("Calculating sqSize max="+sqSizeMax);
	...
	for(var y=0; y<h; y++){
		for(var x=0; x<w; x++){
			...
			//if(DEBUG) console.debug("x="+x+", y="+y+", code["+charIndex+"]= >>"+val+"<<");
...
function formatBits(bitString, chunkSize, chunksPerLine){
	...
	if(DEBUG) console.debug("BEFORE: "+justBits);
 ...
 var charsPerLine = chunksPerLine*(chunkSize+1);
	...
	if(DEBUG) console.debug("AFTER: "+formattedBits);
	return formattedBits;
...
function hexToBin(){
	var allHexDigits = document.getElementById("binaryImage").value.replace(/[^0-9A-F]/gi,"");
	if(DEBUG) console.debug("about to convert HEX2BIN and write to text area");
...
function insertChunkOffsetSpaces(bitsPerChunk, bitText){
 			...
 			for(i=0; i<bitText.length; i+=bitsPerChunk){
 				  if(i%(width*bitsPerChunk)==0){
 				  	if(DEBUG) console.debug("i="+i+" and lineBreak?");
...
function binToHex(){
	...
	var allBits = document.getElementById("binaryImage").value.replace(/[^01]/gi,"");
		if(DEBUG) console.debug("about to convert BIN2HEX and write to text area");
...
function getColorVal2(binVal, bitsPerPixel){
	...
	if(DEBUG) console.debug("bitsPerPixel = "+bitsPerPixel)
	...
	if(bitsPerColor*3 != bitsPerPixel){
		...
		val = parseInt(val);
		if(DEBUG) console.debug("Grayscale! -- "+val);
		return "rgb("+val+", "+val+", "+val+")";
	}
	else{
		if(DEBUG) console.debug("bitsPerColor = "+bitsPerColor)
		...
		if(DEBUG) console.debug("rgb="+R+","+G+","+B);
		...
		if(DEBUG) console.debug("rgbVals="+Rval+","+Gval+","+Bval);
...
function getColorVal(binVal, bitsPerColor){
	...
	if(DEBUG) console.debug("range="+range);
...
//@param canvasId the id of the canvas you want to make a PNG of   сохранение

function showPNG(canvasId){
 	var canvas = document.getElementById(canvasId);
	var ctx=canvas.getContext("2d");
var test = canvas.toDataURL("image/png")
	var W = window.open("about:blank",canvasId,"width="+"300"+", height="+"600"+", left=100, menubar=0, titlebar=0, scrollbars=0");
	W.document.write("<img src='"+test+"' alt='from canvas'/>");
	W.document.write("<style>a { position: relative;  display: inline-block;  width: 10em;  height: 2.5em;  line-height: 2.5em;  vertical-align: middle;  text-align: center;  text-decoration: none;  text-shadow: 0 -1px 1px #777;  color: #fff;  outline: none;  border: 2px solid #F64C2B;  border-radius: 5px;  box-shadow: 0 0 0 60px rgba(0,0,0,0) inset, .1em .1em .2em #800;  background: linear-gradient(#FB9575, #F45A38 48%, #EA1502 52%, #F02F17); } </style><a href="+test+"  download>Скачать картинку</a>");
...
function saveBitsWindow(bitText){
	 ...
	 var myWindow = window.open("", "Bits", "width=200, height=200");
		myWindow.document.write("<p>Ниже приведены биты, созданные из картинки.<br>Вы можете скопировать данные, для сохранения кода каринки.<br>Примечание: вы потеряете эти данные, как только закроете это окно.</p><hr><pre width=60>"+bitText+"</pre>");
		myWindow.focus();
